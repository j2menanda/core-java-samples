...
Sample Explanation
In the first case, the dishes have types of [1, 2, 3, 3, 2, 1][1,2,3,3,2,1], so you'll eat the first 3 dishes, skip the next one as it's another type-3 dish, and then eat the last 2.
...
public class Dishes {
  ...
  public int getMaximumEatenDishCount(int N, int[] D, int K) {
	...
	int count = 0;
    Set<Integer> dishSet = new LinkedHashSet<Integer>();
    
    for(int i=0; i<N; i++){
        if(dishSet.add(D[i])){
            count++;
            if(dishSet.size() > K){
              Iterator<Integer> iterator = dishSet.iterator();
              if(iterator.hasNext()) {
                dishSet.remove(iterator.next());
              }
            }
        }
    }
